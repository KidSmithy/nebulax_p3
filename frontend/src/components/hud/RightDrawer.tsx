...
export const RightDrawer: React.FC = () => {
  ...
  const tabs = [
    ...
    { id: 'ai' as const, label: 'Assessment', icon: Stethoscope },
    { id: 'whatif' as const, label: beginner ? 'Repairs' : 'WHAT-IF', icon: Wrench },
  ];

  return (
    <aside className="absolute top-28 right-4 bottom-20 z-20 transition-colors duration-150 duration-300 pointer-events-none flex flex-col items-end">
      {!isRightDrawerOpen ? (
        <button
          onClick={toggleRightDrawer}
          className="glass-panel-floating px-3 py-2 rounded pointer-events-auto flex items-center space-x-2 text-xs font-mono text-ink-700 hover:text-ink-900 transition-colors duration-150"
        >
          <BarChart2 className="w-4 h-4 text-ink-700" />
          <span>{beginner ? 'OPEN DETAILS' : 'EXPAND COCKPIT'}</span>
          <ChevronLeft className="w-3.5 h-3.5" />
        </button>
      ) : (
        <div className="glass-panel p-3 rounded pointer-events-auto w-[21rem] flex flex-col space-y-2.5 max-h-full">
          {/* Tabs: full labels, active underlined against a full-width hairline */}
          <div className="flex items-center justify-between border-b border-slate-200 shrink-0">
            <div className="flex items-center font-mono text-label">
              {tabs.map((t) => {
                const Icon = t.icon;
                const active = rightDrawerTab === t.id;
                return (
                  <button
                    key={t.id}
                    onClick={() => setRightDrawerTab(t.id)}
                    className={`px-2 py-2 -mb-px flex items-center space-x-1 border-b-2 transition-colors duration-150 ${
                      active
            ...
            <button
              onClick={toggleRightDrawer}
              className="p-1 rounded text-slate-400 hover:text-slate-800 hover:bg-slate-100 transition-colors mb-1"
          ...
          <div className="overflow-y-auto custom-scrollbar pr-0.5 -mr-0.5">
            ...
            {rightDrawerTab === 'telemetry' && (
              <div className="flex flex-col space-y-2.5">
                ...
                <section className="space-y-1.5">
                  <h3 className="text-[9.5px] font-bold text-slate-500 flex items-center gap-1">
                  ...
                  {doorFault && door?.fault_type !== 'NONE' && (
                    <div className="p-1.5 rounded bg-amber-50 border border-amber-200 text-label text-amber-900 leading-relaxed">
                ...
                <section className="space-y-1.5">
                  ...
                  <div className="grid grid-cols-2 gap-1.5">
                    ...
                    <div className="rounded border border-slate-200 bg-slate-50 p-1.5">
                      ...
                      <div className="mt-0.5 text-label font-bold text-slate-700 leading-tight">
                ...
                <section className="space-y-1.5">
                  ...
                  {railClass && (
                    <div className="p-1.5 rounded bg-slate-50 border border-slate-200 text-label text-slate-700 leading-relaxed">
                ...
                <div className="p-2 rounded bg-slate-50 border border-slate-200 text-label flex items-start gap-1.5">
                  <Link2 className="w-3 h-3 text-ink-500 shrink-0 mt-0.5" />
                  <div className="text-slate-700 leading-relaxed">
                    <strong className="text-ink-900">How these connect: </strong>
                    {beginner ? (
                      <>
                ...
                <div className="bg-white p-2 rounded border border-slate-200">
                  <div className="flex items-center justify-between mb-1 text-[9.5px] font-bold text-slate-600">
            ...
            {/* ============================== AI ========================== */}
            {rightDrawerTab === 'ai' && <DiagnosisPanel />}
