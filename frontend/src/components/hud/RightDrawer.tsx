...
export const RightDrawer: React.FC = () => {
  ...
  return (
    <aside className="absolute top-20 right-4 bottom-20 z-20 transition-colors duration-150 duration-300 pointer-events-none flex flex-col items-end">
      {!isRightDrawerOpen ? (
        <button
          onClick={toggleRightDrawer}
          className="glass-panel-glow px-3 py-2 rounded pointer-events-auto flex items-center space-x-2 text-xs font-mono text-red-600 hover:text-red-800 transition-colors duration-150 shadow-xl"
        >
        ...
        <div className="glass-panel p-3 rounded pointer-events-auto w-[21rem] shadow-2xl flex flex-col space-y-2.5 max-h-full">
          {/* Tabs */}
          <div className="flex items-center justify-between border-b border-slate-200 pb-2 shrink-0">
            <div className="flex items-center space-x-1 bg-slate-100 p-0.5 rounded border border-slate-200 font-mono text-label">
              {tabs.map((t) => {
                const Icon = t.icon;
                return (
                  <button
                    ...
                    className={`px-2 py-1 rounded flex items-center space-x-1 transition-colors duration-150 ${
                      rightDrawerTab === t.id
