          ...
          <div className="overflow-y-auto custom-scrollbar">
            {log.length === 0 ? (
              ...
              log.map((entry, i) => {
                ...
                const healthDelta = entry.measured_impact?.changed_metrics.find(
                  (m) => m.path === 'fleet_health_index'
                );
                return (
                  <div
                    key={entry.id}
                    className={`px-2.5 py-2 ${i > 0 ? 'border-t border-slate-100' : ''}`}
                  >
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <span className="text-label font-mono text-slate-400 shrink-0">
                        KP {entry.resolved_at_kp.toFixed(3)}
                      </span>
                    </div>
                    <div className="text-label text-slate-500 font-mono mt-0.5 truncate">
                      {entry.file_name}
                    </div>
                    {repair && (
                    ...
                    {healthDelta && (
                      <div className="text-label font-mono text-status-nominal mt-0.5">
                        Overall health {(healthDelta.before * 100).toFixed(0)}% → {(healthDelta.after * 100).toFixed(0)}%
                      </div>
                    )}
                  </div>
                );
