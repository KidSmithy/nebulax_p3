    ...
    <div className="flex flex-col space-y-2.5">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-1.5">
          <Stethoscope className="w-3.5 h-3.5 text-ink-700" />
        ...
        <div className="flex items-center space-x-1">
          <button
            ...
            className={`text-label font-mono px-1.5 py-0.5 rounded border transition-colors ${
              autoRefresh
          ...
          <button
            ...
            disabled={loading}
            className="p-1 rounded text-slate-500 hover:text-ink-700 hover:bg-red-50 disabled:opacity-40 transition-colors"
      ...
      {aiEnabled === false && (
        <div className="p-2 rounded bg-amber-50 border border-amber-200 text-label text-amber-800 leading-snug">
          The diagnosis service is not configured, so these explanations come from built-in threshold rules
      ...
      {loading && !insight && (
        <div className="p-4 flex flex-col items-center justify-center text-slate-500 space-y-2">
          <Loader2 className="w-5 h-5 animate-spin text-status-fault" />
          <span className="text-label">Reading the live telemetry…</span>
        </div>
      )}
      ...
      {insight && (
        <>
          {/* Headline verdict */}
          <div className={`p-2.5 rounded border ${styles.border} ${styles.bg}`}>
          ...
          {/* Everyday analogy */}
          {insight.analogy && (
            <div className="p-2 rounded bg-sky-50 border border-sky-200 flex items-start space-x-1.5">
              <Lightbulb className="w-3 h-3 text-sky-600 shrink-0 mt-0.5" />
              <p className="text-label text-sky-900 leading-relaxed italic">{insight.analogy}</p>
            </div>
          )}
          ...
          {insight.findings.length > 0 && (
            <div className="space-y-1.5">
              ...
              {insight.findings.map((f, i) => (
                ...
                >
                  <div className="text-label font-bold text-red-700 ">
          ...
          <div className="p-2 rounded bg-slate-50 border border-slate-200">
            ...
            <div className="mt-1 text-label font-mono text-slate-500">
      ...
      <div className="pt-2 border-t border-slate-200 space-y-1.5">
        <div className="flex items-center space-x-1.5">
          <ClipboardCheck className="w-3 h-3 text-ink-700" />
          ...
          <div className="flex flex-wrap gap-1">
            {SUGGESTED_QUESTIONS.map((q) => (
              <button
                ...
                onClick={() => submit(q)}
                disabled={asking}
                className="text-label text-left px-1.5 py-1 rounded bg-slate-50 border border-slate-200 text-slate-600 hover:border-red-300 hover:text-red-700 disabled:opacity-40 transition-colors"
              >
                {q}
              </button>
            ))}
        ...
        >
          ...
          <input
            ...
            maxLength={500}
            className="w-full text-[10.5px] pl-2 pr-7 py-1.5 rounded bg-white border border-slate-300 text-slate-800 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-red-400 focus:border-red-400"
          />
          <button
            type="submit"
            disabled={asking || !question.trim()}
            aria-label="Send question"
            className="absolute right-1 top-1/2 -translate-y-1/2 p-1 rounded text-slate-400 hover:text-ink-700 disabled:opacity-30 transition-colors"
          >
            {asking ? (
              <Loader2 className="w-3.5 h-3.5 animate-spin" />
            ) : (
