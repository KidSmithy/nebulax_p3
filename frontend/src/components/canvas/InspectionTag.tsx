      ...
      <Html center style={{ zIndex: isActive ? 1000 : 10 }}>
        {!isActive ? (
          ...
          <button
            ...
            className="group flex items-center space-x-1.5 px-2 py-0.5 rounded-full bg-white/95 backdrop-blur-md border border-red-500/60 text-slate-800 hover:border-red-600 hover:bg-red-50 shadow-md shadow-red-500/10 transition-all hover:scale-105 cursor-pointer select-none"
          >
            <span className="relative flex h-2 w-2 shrink-0">
              <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-red-400 opacity-75"></span>
              <span className="relative inline-flex rounded-full h-2 w-2 bg-red-600"></span>
            </span>
            <span className="text-[10px] font-mono font-bold tracking-wider uppercase text-slate-850 whitespace-nowrap">
              {beaconLabel}
            </span>
          </button>
        ) : (
          ...
          <div
            ...
            className="w-64 glass-panel p-3 rounded-2xl border-red-400 shadow-2xl text-slate-800 text-xs font-sans pointer-events-auto select-none animate-in fade-in zoom-in-95 duration-150 relative z-50"
          >
            {/* Header */}
            <div className="flex items-center justify-between border-b border-black/5 pb-1.5 mb-2">
              <div className="flex items-center space-x-1.5">
                {type === 'shm' && <Activity className="w-4 h-4 text-red-600" />}
                {type === 'door' && <DoorOpen className="w-4 h-4 text-red-600" />}
                {type === 'acv' && <Wind className="w-4 h-4 text-sky-600" />}
                {type === 'rail' && <GitCommit className="w-4 h-4 text-amber-600" />}
              ...
              <div className="space-y-1.5 font-mono text-[10px]">
                <div className="grid grid-cols-2 gap-1.5">
                  <div className="bg-slate-50 p-1.5 rounded-lg border border-slate-200/70">
                    <div className="text-slate-500 text-[8px] uppercase font-semibold">Vibration RMS</div>
                    <div className="text-red-600 font-bold text-xs">{subsystems.shm.vibration_rms_g} g</div>
                  </div>
                  <div className="bg-slate-50 p-1.5 rounded-lg border border-slate-200/70">
                    <div className="text-slate-500 text-[8px] uppercase font-semibold">Peak Freq</div>
                    <div className="text-slate-900 font-bold text-xs">{subsystems.shm.peak_frequency_hz} Hz</div>
                  </div>
                </div>
                <div className="bg-slate-50 p-1.5 rounded-lg border border-slate-200/70 flex justify-between items-center">
                  <span className="text-slate-500">Critical Node:</span>
                  <span className="text-amber-600 font-bold">{subsystems.shm.critical_weld_node}</span>
                </div>
                <div className="bg-slate-50 p-1.5 rounded-lg border border-slate-200/70 flex justify-between items-center">
            ...
            {type === 'door' && subsystems?.door && (
              <div className="space-y-1.5 font-mono text-[10px]">
                <div className="grid grid-cols-2 gap-1.5">
                  <div className="bg-slate-50 p-1.5 rounded-lg border border-slate-200/70">
                    <div className="text-slate-500 text-[8px] uppercase font-semibold">Motor Current</div>
                    <div className="text-red-600 font-bold text-xs">{subsystems.door.motor_current_amps} A</div>
                  </div>
                  <div className="bg-slate-50 p-1.5 rounded-lg border border-slate-200/70">
                    <div className="text-slate-500 text-[8px] uppercase font-semibold">Cycle State</div>
                ...
                <div className="bg-slate-50 p-1.5 rounded-lg border border-slate-200/70 flex justify-between items-center">
                  ...
                  <span className="text-emerald-600 font-bold">{subsystems.door.ghost_deviation_mm} mm</span>
                </div>
              </div>
            )}
            ...
            {type === 'acv' && subsystems?.acv && (
              <div className="space-y-1.5 font-mono text-[10px]">
                <div className="grid grid-cols-2 gap-1.5">
                  <div className="bg-slate-50 p-1.5 rounded-lg border border-slate-200/70">
                    <div className="text-slate-500 text-[8px] uppercase font-semibold">Delta-T</div>
                    <div className="text-sky-600 font-bold text-xs">{subsystems.acv.delta_temp_c} °C</div>
                  </div>
                  <div className="bg-slate-50 p-1.5 rounded-lg border border-slate-200/70">
                    <div className="text-slate-500 text-[8px] uppercase font-semibold">Power</div>
                ...
                <div className="bg-slate-50 p-1.5 rounded-lg border border-slate-200/70 flex justify-between items-center">
                  ...
                  <span className="text-emerald-600 font-bold">{subsystems.acv.efficiency_rating}</span>
                </div>
                <div className="bg-slate-50 p-1.5 rounded-lg border border-slate-200/70 flex justify-between items-center">
                  <span className="text-slate-500">Refrigerant Leak:</span>
                  <span className={subsystems.acv.fault_type === 'NONE' ? 'text-emerald-600 font-semibold' : 'text-red-600 font-semibold'}>
            ...
            {type === 'rail' && subsystems?.rail_corrugation && (
              <div className="space-y-1.5 font-mono text-[10px]">
                <div className="grid grid-cols-2 gap-1.5">
                  <div className="bg-slate-50 p-1.5 rounded-lg border border-slate-200/70">
                    <div className="text-slate-500 text-[8px] uppercase font-semibold">Roughness</div>
                    <div className="text-amber-600 font-bold text-xs">{subsystems.rail_corrugation.depth_microns} μm</div>
                  </div>
                  <div className="bg-slate-50 p-1.5 rounded-lg border border-slate-200/70">
                    <div className="text-slate-500 text-[8px] uppercase font-semibold">Wavelength</div>
                ...
                <div className="bg-slate-50 p-1.5 rounded-lg border border-slate-200/70 flex justify-between items-center">
                  ...
                  <span className="text-emerald-600 font-bold">{subsystems.rail_corrugation.maintenance_urgency}</span>
                </div>
              </div>
            )}
            ...
            <div className="mt-2.5 pt-2 border-t border-black/5 flex items-center justify-between">
              <button
                ...
                className="w-full py-1.5 px-2.5 rounded-lg bg-red-600 hover:bg-red-500 text-white font-mono text-[10px] font-bold flex items-center justify-center space-x-1 transition-all shadow-md shadow-red-600/30"
              >
